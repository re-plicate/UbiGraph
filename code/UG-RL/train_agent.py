...
def train(worker_id, traffic_light, counter, shared_model, shared_grad_buffers, local_time, son_process_counter,
    ...
    poi_data_shape = env.env_gym.pois_num
    ...
    # -----------------create storage---------------------
    rollout = RolloutStorage(params.max_time_steps, params.mini_batch_num, obs_shape, uav_num,poi_data_shape)
    rollout.to(device)

    # ---------------create local model---------------------
    local_ppo_model = Model(obs_shape, uav_num,poi_data_shape, device)
    ...
    while True:
        ...
        cur_obs, uav_aoi, uav_snr, uav_tuse, uav_effort, cur_poi_data_val = env.reset()

        rollout.after_update(cur_obs, uav_aoi, uav_snr, uav_tuse, uav_effort, cur_poi_data_val)
        ...
        while step < params.max_time_steps:
                ...
                if params.use_rnn:
                    if params.use_spatial_att:
                        value, action, action_log_probs, temporal_hidden_states, spatial_hidden_states = local_ppo_model.act(
                            ...
                            rollout.uav_effort[step].unsqueeze(0),rollout.cur_poi_data_val[step].unsqueeze(0), rollout.temporal_hidden_states[step].unsqueeze(0),
                    ...
                    value, action, action_log_probs = local_ppo_model.act(rollout.obs[step].unsqueeze(0),
                                                                          ...
                                                                          rollout.uav_effort[step].unsqueeze(0),rollout.cur_poi_data_val[step].unsqueeze(0))
                next_obs, reward, done, uav_aoi, uav_snr, uav_tuse, uav_effort,cur_poi_data_val,finished, dead= env.step(
            ...
            cur_poi_data_val = cur_poi_data_val[:256]
            if finished:
                av_reward += (reward + (params.max_time_steps-step) * env.env_gym.sg.V["ADVANCE_END_STEP"] + env.env_gym.sg.V["ADVANCE_END"])
                break
            if dead:
                av_reward += env.env_gym.sg.V["DEAD_PENALTY"]
                break
            ...
            if params.use_rnn:
                if params.use_spatial_att:
                    rollout.insert(next_obs.detach(), uav_aoi.detach(), uav_snr.detach(), uav_tuse.detach(),
                                   uav_effort.detach(),cur_poi_data_val.detach(), action.detach(), action_log_probs.detach(), value.detach(),
        ...
        # if done:
        #     av_reward -= env.env_gym.sg.V["PRECISION"] * (len(env.env_gym.poi_data_pos) - len(visited_pois))
        with torch.no_grad():
            if done:
        ...
        for _ in range(params.ppo_epoch):
            ...
            for samples in data_generator:
                ...
                if params.use_rnn:  # TODO
                    if params.use_spatial_att:
                        obs_batch, uav_aoi_batch, uav_snr_batch, uav_tuse_batch, uav_effort_batch,cur_poi_data_val_batch, action_batch, \
        ...
        if worker_id == 0:
            av_reward_list.append(av_reward_np.item())
            ...
            # for i in range(len(av_reward_list)):
            #     av_reward_list[i] = [arr.item() for arr in av_reward_list[i]]
            # av_reward_list = [arr.item() for arr in av_reward_list]
            save_to_json(av_reward_list,"av_reward.json")
            save_to_json(av_value_loss_list, "critic_loss.json")
            save_to_json(av_policy_loss_list, "actor_loss.json")
            save_to_json(av_ent_loss_list, "entropy_loss.json")
