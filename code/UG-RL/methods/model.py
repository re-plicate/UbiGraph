...
class Model(nn.Module):
    def __init__(self, obs_shape, num_of_uav,poi_data_shape, device, trainable=True, hidden_size=params.temporal_hidden_size):
        '''
            obs_shape:环境信息
            num_of_uav:无人机数量
            device:设备
            hidden_size:时序中隐藏层维度
        '''
        ...
        # actor 输出连续动作分布（高斯分布）
        self.dist_dia = DiagGaussian(hidden_size + num_of_uav * 4+poi_data_shape, params.uav_action_dim * num_of_uav,
        ...
        self.critic = nn.Sequential(
            init_(nn.Linear(hidden_size + num_of_uav * 4 +poi_data_shape, 1))
        )
        self.device = device
    ...
    # 生成动作
    def act(self, inputs, uav_aoi, uav_snr, uav_compl, uav_tc_compl,cur_poi_data_val, temporal_hidden_state=None, mask=None,
        ...
        full_obs_feature = torch.cat(
            [obs_feature, uav_aoi.float(), uav_snr.float(), uav_compl.float(), uav_tc_compl.float(),cur_poi_data_val], dim=1)
        # 估计转态价值
        value = self.critic(full_obs_feature)
        # 输出连续动作分布（高斯分布） size [hidden_size + num_of_uav * 4, params.uav_action_dim * num_of_uav]
        # 4 为 uav_aoi.float(), uav_snr.float(), uav_compl.float(), uav_tc_compl.float()
        dist_dia = self.dist_dia(full_obs_feature)
        action_dia = dist_dia.sample() #从对角高斯分布中采样动作，使得动作空间可微
        # 计算动作在分布中的对数概率
    ...
    # 估计状态价值
    def get_value(self, inputs, uav_aoi, uav_snr, uav_compl, uav_tc_compl,cur_poi_data_val, temporal_hidden_state=None, mask=None,
                  spatial_hidden_state=None):
        if params.use_rnn:
            if params.use_spatial_att:
        ...
        full_obs_feature = torch.cat([obs_feature, uav_aoi, uav_snr, uav_compl, uav_tc_compl,cur_poi_data_val], dim=1)
        value = self.critic(full_obs_feature)
        return value

    # 评估动作
    def evaluate_actions(self, inputs, uav_aoi, uav_snr, uav_compl, uav_tc_compl,cur_poi_data_val, action, temporal_hidden_state=None,
        ...
        full_obs_features = torch.cat([obs_features, uav_aoi, uav_snr, uav_compl, uav_tc_compl,cur_poi_data_val], dim=1)
...
# 集成了卷积网络和循环神经网络，适用于强化学习中的策略网络设计，特别是针对视觉输入的多智能体系统（如无人机控制）
