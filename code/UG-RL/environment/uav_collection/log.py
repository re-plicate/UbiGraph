...
class Log(object):
    ...
    def draw_path(self, env, episode):
        ...
        MOVE_PATH = [[] for _ in range(env.uav_num)]
        DEAD_PATH = [[] for _ in range(env.uav_num)]
        FINISHED_PATH = [[] for _ in range(env.uav_num)]
        ...
        for i in range(env.uav_num):
            for j, pos in enumerate(PATH[i]):
                ...
                elif env.uav_state[i][j] == 3:  # charge power(unnecessary)
                    MOVE_PATH[i].append(pos)
                elif env.uav_state[i][j] == 2:
                    FINISHED_PATH[i].append(pos)
                else:
                    print(" error")
        ...
        for i in range(env.uav_num):
            ...
            if len(POI_PATH[i]) > 0:
                plt.scatter(np.stack(POI_PATH[i])[:, 0], np.stack(POI_PATH[i])[:, 1], color="green", marker='+')

            if len(MOVE_PATH[i]) > 0:
                plt.scatter(np.stack(MOVE_PATH[i])[:, 0], np.stack(MOVE_PATH[i])[:, 1], color=color, marker='.')

            if len(DEAD_PATH[i]) > 0:
                plt.scatter(np.stack(DEAD_PATH[i])[:, 0], np.stack(DEAD_PATH[i])[:, 1], color=color, marker='D')
            if len(FINISHED_PATH[i]) > 0:
                plt.scatter(np.stack(FINISHED_PATH[i])[:, 0], np.stack(FINISHED_PATH[i])[:, 1], color="orange", marker='o')
        ...
        if self.worker_id == 0:
            ...
            if params.trainable is True:
                ...
                save_to_json(self.time_usage_ratio_list, "time_usage_ratio.json")
            ...
            save_to_json(self.data_collection_list, "data_collection.json")
            ...
            save_to_json(self.energy_comsumption_list,"energy_comsumption.json")
            ...
            save_to_json(self.eficiency_list, "eficiency_list.json")
