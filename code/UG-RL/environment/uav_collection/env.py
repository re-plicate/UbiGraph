...
class Env(object):
    def __init__(self, log):
        ...
        self.collect_aoi = self.sg.V['COLLECT_AOI']
        self.poi_radius = self.sg.V["POI_RADIUS"]
        # map obstacle [ 16 , 16 ]
        ...
        self.uav_target_poi = [[0, 0] for _ in range(self.uav_num)]
        # action  [ K , 3 ]
        ...
        self.pois_num = len(self.poi_data_pos)
    ...
    def reset(self):
        ...
        self.finished_uav_list = [False] * self.uav_num
        ...
        self.reset_target_pois()
        # self.uav_target_poi = [[10000, 10000], [0, 0]]

        self.poi_loiter_count = {}
        ...
        return self.get_observation(), np.array(self.uav_aoi_list), np.array(self.uav_snr_list), np.array(
            self.data_completion_ratio_list), np.array(self.collection_effort_ratio_list),np.array(self.cur_poi_data_val)

    def is_any_array_equal(self, poi_pos,other_uav_poi):
        for element in other_uav_poi:
            if np.all(poi_pos == element):
                return True
        return False
    def reset_target_pois(self):
            for i in range(self.uav_num):

                other_uav_poi = self.uav_target_poi[:i] + self.uav_target_poi[i + 1:]
                ...
                for poi_index, (poi_pos, poi_val) in enumerate(zip(self.poi_data_pos, self.cur_poi_data_val)):
                    ...
                    if self.cur_poi_data_val[poi_index] <= self.sg.V['EPSILON']:
                        continue
                    if self.is_any_array_equal(poi_pos, other_uav_poi):
                        continue
                    # cal distance between uav and poi
                    distance = self.cal_distance(self.cur_uav_pos[i], poi_pos)
                    if distance < min_distance:
                        min_distance = distance
                        pos = poi_pos
                self.uav_target_poi[i] = pos

            print("uav_0 targetpoi:",self.uav_target_poi[0],"   uav_1 targetpoi:", self.uav_target_poi[1])
    ...
    def cal_distance(self, pos1, pos2):
        ...
        return distance_unit * self.sg.V['ACTUAL_SIZE']
    ...
    def cal_collect_aoi(self, i, move_aoi):
        collect_aoi = min(self.collect_aoi, self.sg.V['TIME_SLOT'] - move_aoi)
    ...
    def calculate_direction_reward(self, current_pos, next_pos, nearest_poi, min_distance):
        rewards = []
        distances = []
        move_direction = np.array(next_pos[:2]) - np.array(current_pos[:2])
        for poi_index, (poi_pos, poi_val) in enumerate(zip(self.poi_data_pos, self.cur_poi_data_val)):
            # this poi has no data left
            poi_vector = np.array(poi_pos[:2]) - np.array(current_pos[:2])

            if self.cur_poi_data_val[poi_index] <= self.sg.V['EPSILON']:
                rewards.append(0)
                distances.append(0)
                continue
            distance = np.linalg.norm(poi_vector)
            distances.append(distance)
            cos_sim = np.dot(move_direction, poi_vector) / (
                    np.linalg.norm(move_direction) * np.linalg.norm(poi_vector) + 1e-8
            )
            rewards.append(self.sg.V["DIRECTION"] * cos_sim)

        decay_factor = 1.5
        if all(d == 0 for d in distances):
            distance_weights = np.ones(len(distances)) / len(distances)
        else:
            inv_distances = np.array([
                np.exp(-decay_factor * d) if d > 0 else 1.0
                for d in distances
            ])
            distance_weights = inv_distances / inv_distances.sum()

        return np.sum(np.array(rewards) * distance_weights)


    def collect_and_sendback(self, i, uav_pos, move_aoi, collect_aoi, greedy_mode, visited_pois, pre_pos):
        iscollect = False
        ...
        perfect_precision_reward=0
        # if i == 0:
        #     other_uav_poi = self.uav_target_poi[1]
        #     other_uav_pos = self.cur_uav_pos[1]
        #     other_index = 1
        # else:
        #     other_uav_poi = self.uav_target_poi[0]
        #     other_uav_pos = self.cur_uav_pos[0]
        #     other_index = 0
        other_uav_poi = self.uav_target_poi[:i] + self.uav_target_poi[i + 1:]
        min_distance = float('inf')
        pos = None
        for poi_index, (poi_pos, poi_val) in enumerate(zip(self.poi_data_pos, self.cur_poi_data_val)):
            ...
            if self.cur_poi_data_val[poi_index] <= self.sg.V['EPSILON']:
                continue
            if self.is_any_array_equal(poi_pos, other_uav_poi) and len(visited_pois) < self.pois_num:
                continue
            # cal distance between uav and poi
            ...
            if len(visited_pois) < 256:
                if np.all(poi_pos == self.bs_pos[0]):

                        self.uav_state[i].append(2)
                        perfect_precision_reward += self.sg.V["RETURN_REWARD"]
                    # else:
                    #     self.uav_state[other_index][-1]=2
                    #     perfect_precision_reward += self.sg.V["RETURN_REWARD"]
                else:

                    # if distance is within collecting range
                    if distance <= self.poi_radius:
                        if distance <= 20:
                            perfect_precision_reward += self.sg.V["PERFECT_PRECISION"]
                        visited_pois.append(tuple(poi_pos))
                    ...
                    perfect_precision_reward += self.sg.V["RETURN_REWARD"]
        if iscollect:
            if len(visited_pois) ==self.pois_num-1:
                self.uav_target_poi[i] = self.bs_pos[0]
            else:
                self.uav_target_poi[i] = pos
        direction_reward = self.calculate_direction_reward(pre_pos, self.cur_uav_pos[i], self.uav_target_poi[i],
                                                                   min_distance)
        ...
        if num_select_poi > 0:
            ...
            time_usage_ratio = min((collect_aoi + move_aoi) / self.sg.V["TIME_SLOT"], 1.0)
            data_completion_ratio = min((self.sg.V["TIME_SLOT"] - move_aoi - collect_aoi) / (send_back_aoi+ self.sg.V["EPSILON"]), 1.0)
        ...
        assistant_reward = 0.7*direction_reward + perfect_precision_reward

        return total_aoi, suppose_total_aoi, suppose_collect_energy_consuming, suppose_send_back_energy_consuming, assistant_reward, iscollect
    ...
    def calcualte_min_distance_reward(self, i, pre_pos, next_uav_pos, visited_pois):

        min_distance = np.sqrt(np.power(next_uav_pos[0] - self.uav_target_poi[i][0], 2) + np.power(next_uav_pos[1] - self.uav_target_poi[i][1], 2))
        if min_distance < float('inf'):
            cur_distance = np.sqrt(np.power(pre_pos[0] - self.uav_target_poi[i][0], 2) + np.power(pre_pos[1] - self.uav_target_poi[i][1], 2))
            # if min_distance <= (self.max_distance/self.sg.V["ACTUAL_SIZE"]):
            distance_change = cur_distance - min_distance
            min_distance_reward = self.sg.V['MIN_DISTANCE_REWARD'] * distance_change
            # print("min_distance_reward", min_distance_reward)
            # else:
            #     min_distance_reward = self.sg.V['MIN_DISTANCE_REWARD'] * (-0.1)

            poi_key = tuple(self.uav_target_poi[i])
            if hasattr(self, 'poi_loiter_count'):
                if poi_key not in self.poi_loiter_count:
                    self.poi_loiter_count[poi_key] = 0
                if abs(distance_change) < 0.2:
                    self.poi_loiter_count[poi_key] += 1
                    if self.poi_loiter_count[poi_key] > 3:
                        loitering_penalty = self.sg.V['LOITERING_PENALTY'] * (
                                    self.poi_loiter_count[poi_key] - 3)
                        min_distance_reward -= loitering_penalty
                else:
                    self.poi_loiter_count[poi_key] = 0
    ...
    def step(self, action, current_step=None, greedy_mode=False, visited_pois=None, num_steps=None):
        if self.sg.V["DEBUG_MODE"]:
            print("\n************* STEP %d" % current_step, " ****************")
        ...
        for i in range(self.uav_num):
            ...
            if self.finished_uav_list[i]:
                # self.uav_state[i].append(2)
                continue
            if self.uav_state[i][-1] == 2:
                self.finished_uav_list[i] = True
                continue
            ...
            self.uav_penalty_list[i] += self.sg.V["DISTANCE_RATIO"] * move_distance
            if self.sg.V["DEBUG_MODE"]:
            ...
            uav_total_aoi, suppose_total_aoi, suppose_collect_energy_consuming, suppose_send_back_energy_consuming,assistant_reward,iscollect \
                = self.collect_and_sendback(i, self.cur_uav_pos[i], move_aoi, collect_aoi, greedy_mode,visited_pois, pre_pos)
            if self.uav_state[i][-1] != 2:
                if iscollect:
                    self.uav_state[i].append(1)  # uav collects data
                else:
                    self.uav_state[i].append(3)
            min_distance_reward = self.calcualte_min_distance_reward(i, pre_pos, next_uav_pos, visited_pois)

            assistant_reward -= 0.05*self.sg.V['PRECISION'] * (current_step / (num_steps*2)) ** 2 * (
                        len(self.poi_data_pos) - len(visited_pois))
            assistant_reward += 0.3*min_distance_reward
            ...
            visited_ratio = len(visited_pois) / len(self.poi_data_pos)

            bonus_factor = 1
            if self.step_uav_data_collection[i] > 0:

                uav_reward = (self.sg.V['PRECISION'] * bonus_factor *self.collection_effort_ratio_list[i]  * \
                             self.step_uav_data_collection[i] + assistant_reward)/ (total_energy_consuming * self.sg.V['NORMALIZE'])

            else:
                uav_reward = 0 + assistant_reward
        ...
        return observation, total_uav_reward, done, np.array(self.suppose_uav_aoi_list), np.array(
            self.uav_snr_list), np.array(self.data_completion_ratio_list), np.array(self.collection_effort_ratio_list), np.array(self.cur_poi_data_val)
