...
def buildModel( method, clusterId, X, y ):
    ...
    if method == 'dt':
        ...
        modelBest = RandomizedSearchCV(model, cv = 3, param_distributions=param_grid_dt, n_iter=10)
        ...
        modelBest = GridSearchCV(model, cv = 3, param_grid = param_grid_xgb)
        ...
        for index, row in similarClusters.iterrows():
            ...
            for modelName, modelContent in models.items():
                ...
                if len(similarClusterData.index) == 0:
                    ...
                    if not nodb:
                        #print(f"$clusterId: {type(clusterId)} $runTimestamp: {type(runTimestamp)} $simClusterId: {type(rmse)} $timeElapsed[modelName]): {type(timeElapsed[modelName])} $trainingScores[modelName]: {type(trainingScores[modelName])} $len(clusterDataframe.index): {type(len(clusterDataframe.index))}")
                        # Write model info into the database
                        # rmse 和 clusterId 数据类型是 np.int64 或者 np.float64 ，而 psycopg2.ProgrammingError: can't adapt type 'numpy.int64'，
                        # 因此需要转为python的本地类型，通过 item() 方法即可
                        stmt = modelsTable.insert().values(clusterid = clusterId.item(),
                                                           data_points = len(clusterDataframe.index),
                                                    run_timestamp = runTimestamp, similar_clusterid = simClusterId,
                                                    model_name = modelName, training_error = trainingScores[modelName].item(),
                                                    error = rmse.item(), error_type = 'RMSE',
